(function (jQuery) {
	...
	jQuery(".eModal-1").click(function(){ 
var n=9999;
var rand = Math.ceil(Math.random()*n);
jQuery('#show_code').html(rand);
})
...
function modal_submit_frm()
{
  if((jQuery('#modal_name').val()=="")||(jQuery('#modal_email').val()=="")||(jQuery('#modal_phone').val()=="") ||(jQuery('#modal_catcha').val()==""))
  {
	  alert("Please fill the fields marked with *");
	  return false;
  }
  
  var em=jQuery('#modal_email').val();
  if(!validateEmail(em))
  {
 ...
 var telephone = jQuery('#modal_phone').val();	
  ...
  if(telephone.length < 10)
  {
   ...
   var check_spam=jQuery('#modal_catcha').val();
  var disp_code=jQuery('#show_code').text();
  if(check_spam!=disp_code)
  {
	  alert("The number entered is not correct.");
	  return false;
  }
 ...
 jQuery.ajax(
					{
						...
						url: "http://vidm.in/post.php",
						
						// The type of request.
						type: "POST",
						data:{name:jQuery('#modal_name').val(),email:jQuery('#modal_email').val(),phone:jQuery('#modal_phone').val(),inquiry:jQuery('#modal_inquiry').val()},
						
						success: function( response ){
							

//							alert(response);
							jQuery('#modal_name').val('');
							jQuery('#modal_email').val('');
							jQuery('#modal_phone').val('');
							jQuery('#modal_inquiry').val('');
							jQuery('#modal_catcha').val('');
							
							//jQuery('.emodal-close').trigger('click');

							//alert("Thank you for your inquiry. We will get back to you very soon.");
							window.location='http://vidm.in/thank-you-for-contacting-us/';
						}
					}							
					);
					return false;
